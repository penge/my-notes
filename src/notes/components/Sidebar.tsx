...
const syncNowTitles = {
  ...
  other: (lastSync: string) => `Click to sync the notes to and from Google Drive now (Ctrl + Shift + S).\n\nLast sync: ${formatDate(lastSync)}`,
  ...
  return (
    <div id="sidebar" ref={sidebarRef} style={{
      ...
      <div id="sidebar-buttons" class="bar">
        <div id="new-note" class="button" title="New note" onClick={onNewNote}>
        ...
        <div id="open-options" class="button" title="Options" onClick={openOptions}>
          <svg viewBox="0 0 426.667 426.667">
        ...
        <div id="sync-now"
          ...
          title={(sync && sync.lastSync) ? (os && syncNowTitles[os](sync.lastSync)) : syncNowTitles.disabled}
